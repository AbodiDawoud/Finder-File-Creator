    ...
    static func resolvedContent(for template: TemplateDefinition, folderName: String, now: Date = Date()) -> String {
        ...
        return replaceTokens(
            ...
            values: [
                ...
                "{{time}}": timeFormatter.string(from: now),
                "{{year}}": String(Calendar.current.component(.year, from: now)),
                "{{month}}": String(format: "%02d", Calendar.current.component(.month, from: now)),
                "{{day}}": String(format: "%02d", Calendar.current.component(.day, from: now)),
                "{{hour}}": String(format: "%02d", Calendar.current.component(.hour, from: now)),
                "{{minute}}": String(format: "%02d", Calendar.current.component(.minute, from: now)),
                "{{second}}": String(format: "%02d", Calendar.current.component(.second, from: now)),
                "{{user}}": NSUserName(),
